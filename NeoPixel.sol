...
contract ERC20 is IERC20, Ownable {
    ...
    function _issue(address account, uint256 amount) internal {
        require(account != address(0), "TokenFactory: issue to the zero address");
    ...
    function _redeem(address account, uint256 amount) internal {
        require(account != address(0), "TokenFactory: redeem from the zero address");
...
contract TokenFactoryAdminRole is Ownable {

    using Roles for Roles.Role;

    event TokenFactoryAdminRoleAdded(address indexed account);

    event TokenFactoryAdminRoleRemoved(address indexed account);

    Roles.Role private _TokenFactoryAdmins;

    constructor () internal {
        _addTokenFactoryAdmin(msg.sender);
    }

    modifier onlyTokenFactoryAdmin() {
        require(isTokenFactoryAdmin(msg.sender), "TokenFactoryAdminRole: caller does not have the TokenFactoryAdmin role");
        _;
    }

    function isTokenFactoryAdmin(address account) public view returns (bool) {
        return _TokenFactoryAdmins.has(account);
    }

    function addTokenFactoryAdmin(address account) public onlyOwner {
        _addTokenFactoryAdmin(account);
    }

    function removeTokenFactoryAdmin(address account) public onlyOwner {
        _removeTokenFactoryAdmin(account);
    }

    function renounceTokenFactoryAdmin() public {
        _removeTokenFactoryAdmin(msg.sender);
    }

    function _addTokenFactoryAdmin(address account) internal {
        _TokenFactoryAdmins.add(account);
        emit TokenFactoryAdminRoleAdded(account);
    }

    function _removeTokenFactoryAdmin(address account) internal {
        _TokenFactoryAdmins.remove(account);
        emit TokenFactoryAdminRoleRemoved(account);
    }
}

contract TokenFactory is ERC20, TokenFactoryAdminRole {

    function issue(address account, uint256 amount) public onlyTokenFactoryAdmin returns (bool) {
        _issue(account, amount);
        return true;
    }

    function redeem(address account, uint256 amount) public onlyTokenFactoryAdmin returns (bool) {
...
contract NeoPixel is ERC20, ERC20Pausable, TokenFactory, Blacklist {
